"""
generate_calendar_refresh_token.py — Console OAuth flow for Calendar access.

Sibling to generate_gmail_refresh_token.py, same pattern, same GCP OAuth
client (GMAIL_OAUTH_CLIENT_ID/_SECRET — Calendar just needs its own
consented scope and refresh token, not a separate OAuth app). Enable the
Calendar API on that same GCP project before running this.
"""
...
def _get_env_path() -> Path:
    # backend/scripts/generate_calendar_refresh_token.py -> repo root/.env
...
def main() -> None:
    ...
    _upsert_env_var(env_path, "GOOGLE_CALENDAR_OAUTH_REFRESH_TOKEN", refresh_token)

    print("\nSuccess: Calendar refresh token generated.")
    print(f"Saved GOOGLE_CALENDAR_OAUTH_REFRESH_TOKEN in: {env_path}")
