...
def process_escalation(file_path: Path):
    ...
    try:
        ...
        logger.info(f"Executing L2 Intent: {intent_description[:100]}...")
        ...
        if result.returncode == 0:
            ...
            # Mark file as completed by renaming or deleting
            file_path.rename(file_path.with_suffix(".completed"))
        else:
            logger.error(f"L2 execution failed: {result.stderr}")
